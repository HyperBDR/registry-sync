...
`images.yaml` is the single source of truth for images, sources, and tags. Nothing else should need to change to add, remove, or retag a normal synced image. `config/skopeo-fallback-images.txt` is the narrow exception for images requiring a different copy client because of target-registry compatibility.
...
- **`scripts/sync.sh`** — flattens `images.yaml` via mikefarah's Go `yq eval` syntax into `regsync` image entries and runs `regsync once`. GitHub Actions installs the pinned `regsync` release with the official `regclient/actions/regsync-installer` action; local runs fall back to the same release download. Credentials are written only to temporary `regsync` and Skopeo auth files and removed on exit. It sets `blobMax: -1` for Aliyun targets to prefer monolithic uploads before regsync's fallback to chunked uploads. Entries in `config/skopeo-fallback-images.txt` are copied with `skopeo copy --all` instead, retaining all platforms. Any failed sync entry makes the script fail.
